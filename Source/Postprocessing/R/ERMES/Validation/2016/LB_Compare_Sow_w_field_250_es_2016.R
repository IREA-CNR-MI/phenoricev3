...
# es_grid_reduced = file.path(out_folder,'Accessory','Grid_MODIS_250_reduced.shp')
...
levels(es_shape_14@data$sowing_met) = c("Water","Water","Unknown")
crop_rast_14 = crop(es_rast_14, extent(es_grid_250))
es_shape_14@data$sowing_met[which(is.na(es_shape_14@data$sowing_met))] = 'Water'
...
levels(es_shape_15@data$sowing_met) = c("Water","Water","Unknown")
crop_rast_15 = crop(es_rast_15, extent(es_grid_250))
es_shape_15@data$sowing_met[which(is.na(es_shape_15@data$sowing_met))] = 'Water'
...
levels(es_shape_16@data$sowing_met) = c("Water","Water","Unknown")
...
es_stats_14_15_16 = ddply(subset(es_250_melt,counttot > 125), .(variable,Year) ,summarize, count = length(value),avg = mean(value, na.rm = T), stdev = sd(value, na.rm = T), mae = mean(abs(value),na.rm = T))
save(es_250,es_250_melt, es_stats_14_15_16, file = file.path(out_folder,'ES_val_250.RData'))
