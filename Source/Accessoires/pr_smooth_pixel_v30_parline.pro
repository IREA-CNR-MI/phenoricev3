  ...
  FOR line = 0, n_elements(lines)-1 DO BEGIN
    ...
    IF count_lc_ok GT 0 THEN BEGIN ; check on "land cover" mask. If no pixel in the line are "good", skip the line
      ...
      FOR ind_pixel = 0, n_elements(lc_ok)-1 DO BEGIN
        ...
        IF (mapscape EQ 0) THEN nodata_pos = where(vi_pix EQ 32767, count_NA) ELSE nodata_pos = where(vi_pix EQ -3000, count_NA)  
        ...
        IF count_NA LT 10 THEN BEGIN          ; otherwise, perform smoothing
          ...
          smooth_pix = pr_smooth_pix(vi_pix, qa_pix, doy_pix, win_dim_r, win_dim_l, nb, doys_reg)
...
; :Author: lb
;-
FUNCTION pr_smooth_pix, vi_pix, qa_pix, doy_pix, win_dim_r, win_dim_l, nb, doys_reg
  COMPILE_OPT idl2
