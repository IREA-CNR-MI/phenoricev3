FUNCTION pr_process_v30_parline, opts, lines, data_lc, data_VI, data_smooth, $
  data_DOY, nb, ncols, doys_reg, years, proc_year, prev_y

  COMPILE_OPT idl2

  ;- ------------------------------------------------------------------
  ; All this function only prepares the inputs for the pr_smooth_pixel
  ...
  out_RICE_MIN_ARRAY      = bytarr(nb,ns)         ; vettore RICE-MIN (useful for checks - useless for users)
  ...
  FOR line = 0, n_elements(lines)-1 DO BEGIN
    ...
    IF count_lc_ok GT 0 THEN BEGIN ; check on "land cover" mask. If no pixel in the line are "good", skip the line
      ...
      FOR ind_pixel = 0, n_elements(lc_ok)-1 DO BEGIN
        ...
        smooth_pix = data_smooth [*,pixel, line]
        doy_pix = data_DOY [*,pixel,line]
        lst_pix = 0.02*data_LST [*,pixel,line]-273.15
        ...
        IF (mapscape EQ 0) THEN nodata_pos = where(smooth_pix EQ 32767, count_NA) ELSE nodata_pos = where(smooth_pix EQ -3000, count_NA)
