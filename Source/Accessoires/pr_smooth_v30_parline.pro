...
; :Author: lb
;-
FUNCTION pr_smooth_v30_parline, opts, lines, data_lc, data_VI, data_QA,data_DOY, data_smooth, nb, ncols, doys_reg, years, prev_y, smooth_flag, nb_out

  COMPILE_OPT IDL2
;  undefine, proc_year
  ;- ------------------------------------------------------------------
  ; This function cycles on lines and prepares the inputs for the pr_smooth_pixel
  ; function, which is at the bottom of the file ! 
  ;- ------------------------------------------------------------------
;  if (smooth_flag EQ 1) THEN BEGIN
;    smooth_matrix = data_smooth         ; If smoothed file was already existing get the data from the caller
;  endif else begin
;    smooth_matrix = intarr(nb, ncols, n_elements(lines))+32767   ; Otherwise initialize to NODATA
;  endelse
  
  smooth_matrix = intarr(nb, ncols, n_elements(lines))+32767   ; Otherwise initialize to NODATA
  out_matrix = intarr(nb_out, ncols, n_elements(lines))+32767
    ...
    IF count_lc_ok GT 0 THEN BEGIN ; check on "land cover" mask. If no pixel in the line are "good", skip the line
      ...
      FOR ind_pixel = 0, n_elements(lc_ok)-1 DO BEGIN
        ...
        IF count_NA LT 10 THEN BEGIN          ; otherwise, perform smoothing
          ...
          IF (smooth_flag EQ 1) then begin
            smooth_pix = smooth_matrix[*, pixel, line]
          ENDIF else begin
            smooth_pix = pr_smooth_pix(opts, vi_pix, qa_pix, doy_pix, nb, doys_reg)
            smooth_matrix[*, pixel, line] = fix(smooth_pix)
          Endelse
...
;          smooth_matrix[*, line, pixel] = 32767
          out_matrix [*, line, pixel] = 32767

        ENDELSE

      ENDFOR ; end cycle on "good pixels

    ENDIF ELSE BEGIN  ; Endif on check if all line is at 0 in the LC mask
      
      out_matrix [*, line, pixel] = 32767
;      smooth_matrix[*,*,line] = 32767   ; Put result of smoothed line in correct position
  ...
  return, smooth_matrix   ; send back result to the calling oBridge
...
; :Author: lb
;-
FUNCTION pr_smooth_pix, opts, vi_pix, qa_pix, doy_pix, nb, doys_reg
  COMPILE_OPT idl2
