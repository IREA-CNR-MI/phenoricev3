...
FUNCTION pr_crop_stage_min_v30, opts, smooth_pix, NDFI_pix, LST_pix, doy_pix, min_array, max_array, $
  ...
  IF count_mins NE 0 THEN BEGIN  ; Is there at least one min ? If so, then process
    ...
    IF (opts.check_arr_min[0] EQ 1) THEN BEGIN
      vi_min_checks = (smooth_pix [pos_mins] GT opts.VI_TR_MIN)
      if (min(vi_min_checks)) EQ 1 then return, 200  ; if no mins satisfy all criteria, return to caller with code 200 (tells caller to 
    ENDIF ELSE vi_min_checks =bytarr(count_mins)
    ...
    IF (opts.check_arr_min[1] EQ 1) THEN BEGIN
      ...
      vi_grow_checks = (total(der_pos_pix[growth_indexes],1) LT opts.growth_opt[1])
      if (min(vi_grow_checks + vi_min_checks)) EQ 1 then return, 200 ; if no mins satisfy all criteria, return to caller with code 200 (tells caller to skip pixel)
    ...
    IF (opts.check_arr_min[2] EQ 1) THEN BEGIN
      ...
      if (min(vi_grow_checks + vi_min_checks + flood_checks)) EQ 1 then return, 200 ; if no mins satisfy all criteria, return to caller with code 200
    ...
    IF (opts.check_arr_min[3] EQ 1) THEN BEGIN
      ...
      maxs_checks          = (total((max_array [max_check_vi_indexes] EQ 1 ),1) EQ 0)
      if (min(vi_grow_checks + vi_min_checks + flood_checks + maxs_checks)) EQ 1 then return, 200 ; if no mins satisfy all criteria, return to caller 
    ENDIF ELSE maxs_checks = bytarr(count_mins) ; array of length n_mins

    ;fifth check: Minimum in "good " season according to LST
    IF (opts.check_arr_min[4] EQ 1) THEN BEGIN
      LST_checks = (LST_pix [pos_mins] LT opts.lst_thresh)
      if (min(vi_grow_checks + vi_min_checks + flood_checks + maxs_checks + LST_checks)) EQ 1 then return, 200 ; if no mins satisfy all criteria, return
    ENDIF ELSE LST_checks = bytarr(count_mins)

    checks_mins_tot = vi_min_checks + vi_grow_checks + flood_checks + maxs_checks + LST_checks
    min_array [pos_mins] = (1 + checks_mins_tot ) > 0

    ; Return the minimum of the check array ( = 0 only if some min survived !!!!)
    return, min(checks_mins_tot)

  ENDIF ELSE BEGIN

    return, 200                    ; Return 200 if no mins identified in the original TS
