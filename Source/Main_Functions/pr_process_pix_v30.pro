...
;- ---------------------------------------------------------------- - 
  ...
  ; Perform a basic smoothiing of LST values (removes spikes and NODATA)
    ...
    IF check_maxs EQ 0 THEN BEGIN ; Start if cycle - skips all further processing if all maxs already removed
      ...
      ;- corresponding to rice seasons int he selected seasons and get their dates
      ;- --------------------------------------------------------- ;
      ...
      FOREACH season, opts.selquarts, ind_season DO BEGIN
        ...
        ;- --------------------------------------------------------- ;
        ; If a valid doy repo0rted for max in the quarter, compute pheonometrics
        ;- --------------------------------------------------------- ;
        IF out_maxdoy[ind_season] NE -999 THEN BEGIN    
          ...
          ; removfed ALL checks. Now if the time series ends before reaching EOS, the EOS 
          ; is just kept to -999
          ;- --------------------------------------------------------- ;
          ...
          out_int [ind_season]          = total(smooth_pix[pos_min:pos_max])/10  ; integral of EVI
